...
using Celeste;
using Celeste.Mod;
using MadelineParty.Board.GreenSpace;
...
namespace MadelineParty.Board
{
    public class BoardController : Entity
    {
        ...
        public override void Awake(Scene scene)
        {
            ...
            else
            {
                ...
                if (heartStartID < 0)
                {
                    ...
                    heartStartID = possibleHeartSpaces[GameData.Instance.Random.Next(possibleHeartSpaces.Count)].ID;
                }
                ...
                case BoardStatus.PLAYERMOVE:
                    if (playerMoveProgress == playerMoveDistance // If we've hit our destination
                            || playerMoveProgress + 1 < playerMovePath.Count && playerMovePath[playerMoveProgress + 1].ID == GameData.Instance.heartSpaceID
                                && GameData.Instance.heartBlocks.Count > 0)
                    { // Or if we're blocked by a heart block
                    ...
                    if (CurrentPlayerToken.Position.Equals(approaching.screenPosition))
                    {
                        ...
                        else if (CurrentPlayerToken.currentSpace.type == BoardSpaceType.ImmediateEvent)
                        { // If this is an auto green space
                            status = BoardStatus.WAITING;
        ...
        public IEnumerator InitiateMinigame()
        {
            ...
            {
                List<LevelData> minigames = GameData.Instance.GetAllUnplayedMinigames(level, new()
                {
        ...
        public override void Render()
        {
            ...
            if (lineRenderTarget == null || lineRenderTarget.Target.IsContentLost)
            {
        ...
        public static void GenerateTurnOrderRolls()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                turnOrderRolls[i] = list[GameData.Instance.Random.Next(list.Count)];
